...
| `create_note` | Writes a note body with the model, then appends it to `/Shortcuts/IrisOKF/notes.md` via `is.workflow.actions.file.append`. The Apple Notes create action is an App Intent whose descriptor Cherri cannot emit (iOS reports "action could not be found"), so notes are saved to the hands-free Files store instead. | Files/iCloud Drive access. | Implemented; device path/key validation required. |
...
| `quick_journal` | Generates a short journal entry with the model and appends it to `/Shortcuts/IrisOKF/journal.md` via `is.workflow.actions.file.append` (Files store, not Apple Notes; see `create_note`). | Files/iCloud Drive access, NVIDIA key. | Implemented; device path/key validation required. |
...
| `memory_list` | Reads the memory log file (`/Shortcuts/IrisOKF/log.md`) via Get File and returns its text. Does not list the folder: Get Folder Contents needs a folder object and throws "File Is Not a Folder" on a text path. | Files/iCloud Drive permission. | Implemented; device path/key validation required. |
| `memory_status` | Reads the memory log file (`/Shortcuts/IrisOKF/log.md`) via Get File and reports whether memory has content. Folder is created once by the setup primer (Create Folder), not listed. | Files/iCloud Drive permission. | Implemented; device path/key validation required. |
